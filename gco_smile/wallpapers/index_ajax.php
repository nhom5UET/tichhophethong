...
?>&nbsp;&nbsp;'</script>
<?php
	} elseif(!isset($login['check_session'])&&!isset($_POST[login])&&!isset($chikitsa['check_chikitsa'])&&!isset($hvol['check_login'])) {//!isset($_POST[login])
?>
...
session_start();
	...
	if(isset($login['check_user'])){
        $username = $login['check_user'];
        $password = $login['check_pass'];
    }else{
        $username = "admin";
        $password = "huyhoangk57";
    }

		$nivel = 'Funcionario';
		$row = mysql_fetch_array(mysql_query("SELECT * FROM `funcionarios` WHERE `usuario` = '$username'"));
		if($row[nome] == "") {
			$nivel = 'Dentista';
			$row = mysql_fetch_array(mysql_query("SELECT * FROM `dentistas` WHERE `usuario` = '$username'"));
			if($row[nome] == "") {
