            ...
            .$this->strFormTag.'
              ...
              <tr>
                 <td class="enpRpt" align="right" width="'.$this->lSearchTableWidth.'">
                    First few letters of <b>Patient\'s Last Name</b>:
                 </td>
                 <td class="enpRpt">
                    <input type="text" size="5" maxlength="20" name="txtSearch" value="" id="txtSearchID">
                    <i><small>(name must be in the Patient table)</small></i>
                 </td>
              </tr>
   ...
   //-----------------------------------------------------------------------
      ...
      if ($numRows > 0){
          foreach ($query->result() as $row)   {
            ...
            $clsResult->strResult =
                         '<b>'.htmlspecialchars($row->strLName.', '.$row->strFName.' '.$row->strMName).'</b><br>'
                         .'birthdate: '.$row->dteBirth;
