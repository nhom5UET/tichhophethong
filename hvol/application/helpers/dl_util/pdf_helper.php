...
/*
function lMultiCellHeight($cOpts){
*/
/*
   $cOpts = new stdClass;
   $cOpts->pageOrientation ('Portrait', etc)
   $cOpts->units ('pt', etc)
   $cOpts->paperType (LETTER, A4, etc)
   $cOpts->fontFamily
   $cOpts->fontStyle
   $cOpts->fontSize
   $cOpts->strText
   $cOpts->cellX
   $cOpts->cellWidth
   $cOpts->textAlign ('C', 'L', etc)
   $cOpts->lineHeight   (something like fontsize+1)
   $cOpts->pageHeight
   $cOpts->pageWidth
   $cOpts->topMargin
   $cOpts->bottomMargin  (pts up from bottom)
   $cOpts->leftMargin
   $cOpts->rightMargin    (pts from right edge of paper)
   
*/
/*
   require_once('./application/libraries/fpdf/fpdf.php');

   $myPDF = new FPDF($cOpts->pageOrientation, $cOpts->units, $cOpts->paperType);
   $myPDF->AddPage();

   $myPDF->SetFont($cOpts->fontFamily, $cOpts->fontStyle, $cOpts->fontSize);
   $myPDF->SetMargins($cOpts->leftMargin, $cOpts->topMargin, $cOpts->rightMargin);  
   $myPDF->SetAutoPageBreak(true, $cOpts->pageHeight-$cOpts->bottomMargin);
   $myPDF->SetXY($cOpts->cellX, $cOpts->topMargin);
   
$lPageNum = $myPDF->PageNo();   
...
.":\$lPageNum = $lPageNum <br></font>\n");
   
   
   $myPDF->MultiCell($cOpts->cellWidth, $cOpts->lineHeight, $cOpts->strText, 0, $cOpts->textAlign);
   $lNewY = $myPDF->GetY() - $cOpts->topMargin;
   $lPageNum = $myPDF->PageNo();
   $lAddPages = $lPageNum-1;
$zzzlPos = strrpos(__FILE__, '\\'); $zzzlLen=strlen(__FILE__); echo('<font class="debug">'.substr(__FILE__, strrpos(__FILE__, '\\',-(($zzzlLen-$zzzlPos)+1))) .': '.__LINE__
.":\$lAddPages = $lAddPages, \$lNewY = $lNewY <br><br></font>\n");
   
   return($lNewY + ($lAddPages * ($cOpts->pageHeight - ($cOpts->topMargin + $cOpts->bottomMargin))));
}
*/
