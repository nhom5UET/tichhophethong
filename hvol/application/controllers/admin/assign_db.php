...
class assign_db extends CI_CONTROLLER {
   ...
   //---------------------------------------------------------------------
      ...
      $this->form_validation->set_error_delimiters('<br><span class="formErrorInstall">', '</span>');
		$this->form_validation->set_rules('dbName',     '<b>Database Name</b>', 'trim|required');
		$this->form_validation->set_rules('dbHostName', '<b>Host Name</b>',     'trim|required');
		$this->form_validation->set_rules('dbUserName', '<b>Database User Name</b>', 'trim|required');
		$this->form_validation->set_rules('dbPassword', '<b>Database Password</b>', 'trim|required');
		$this->form_validation->set_rules('passcode',   '<b>Patient Encryption Passcode</b>', 'trim|required|min_length[10]');
		...
		if ($this->form_validation->run() == FALSE){
         ...
         if (validation_errors()==''){
            if ($bConnectErr || $bDBSelErr || $bDBOtherErr){
               ...
               $displayData['formData']->passcode     = $_SESSION[CS_NAMESPACE.'install']->passcode;
            }else {
            ...
            $displayData['formData']->passcode     = set_value('passcode');
         }
         ...
         $strPasscode = $_SESSION[CS_NAMESPACE.'install']->passcode   = trim($_POST['passcode']);
   ...
   //---------------------------------------------------------------------
      ...
      $this->clsIDB->updateDBConfigFile($strDBName, $strDBHost, $strUserName, $strPWord, $strPasscode);
      if ($this->clsIDB->bError){
         $_SESSION[CS_NAMESPACE.'install']->strError = $this->clsIDB->strErrMsg;
         redirect('admin/assign_db/dbform/false/false/true');
      }
