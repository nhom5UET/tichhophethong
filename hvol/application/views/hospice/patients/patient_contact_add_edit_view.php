...
//   echoT(strDatePicker('datepicker1', false));
   ...
   if ($bNew){
      ...
      $strPID = str_pad($lPConID, 5, '0', STR_PAD_LEFT);
   }

   $attributes = array('name' => 'frmEditCon', 'id' => 'frmAddEdit');
   echoT(form_open('hospice/patients/patient_contact/addEditContact/'.$lPConID.'/'.$lPatientID));


   if ($bNew){
      $strTitle  = "Add New Patient Contact";
      $strButton = "Add Patient Contact";
   }else {
      $strTitle  = "Update Patient Contact Record";
      $strButton = "Update";
   }   
   ...
   echoT($clsForm->strLabelRow('patientContactID',  $strPID, 1));
   ...
   echoT($clsForm->strGenericTextEntry('Last Name', 'txtLName', true, $formData->txtLName, 30, 30));
      ...
      // Relationship
   $clsForm->strExtraFieldText = form_error('ddlRelationship');
   echoT($clsForm->strLabelRow('Relationship', $formData->ddlRelationship, 1));
   ...
   echoT(form_close('<br>'));
   echoT('<script type="text/javascript">frmEditCon.addEditEntry.focus();</script>');
