 ...
 * @version
 */
public class RegPatientsReport extends HttpServlet {
    ...
    public JFreeChart getBarChart() {
     ...
     RegPatients regPts = new EJBAccessPoint().lookupRepEmgBean().RegPatientBw(frmDate,tDate);

     
        
       
        dataset.setValue("Males",regPts.getMales());
        dataset.setValue("Females",regPts.getFemales());
            
        } catch (Exception e) {
            System.out.println("Error in " + getClass().getName() + "\n" + e);
        }

        return ChartFactory.createPieChart3D("Registered Patients Male & Female \n" + "From: " + fromDate +"   To: " + toDate, dataset,
                                             true, true, false);
