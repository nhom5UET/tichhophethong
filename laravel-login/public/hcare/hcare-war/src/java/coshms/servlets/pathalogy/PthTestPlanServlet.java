/*
 * PthTestPlanServlet.java
 *
 * Created on May 21, 2006, 4:37 PM
 */
 ...
 * @version
 */
public class PthTestPlanServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(userId == 0){
            ...
            String pthPlan=(String)session.getAttribute("pthPlan");
            if(pthPlan==null){
                AuthenticationRemote aRemote = new coshms.util.administration.LookupService().lookupAuthenticationBean();
                authorized=aRemote.isAuthorized(userId,"pthPlan");
                if (authorized){
                    session.setAttribute("pthPlan","yes");
                }else if(!authorized){
                    session.setAttribute("pthAddAndEditTest","no");
                    response.sendRedirect("login.jsp");
                }
            }else if(pthPlan.equals("no")){
                response.sendRedirect("login.jsp");
            }
        }
        ...
        try {
            PthTestPlanInfo pthTestPlan = (PthTestPlanInfo)pthRemoteSB.getPthTestPlan();
...
//			 float height = document.getPageSize().height();
            ...
            table3.addCell(new Paragraph(Integer.valueOf(pthTestPlan.getTestRequested()).toString(), font8));
            table3.addCell(new Paragraph("Urgent Tests ", font8));
            table3.addCell(new Paragraph(Integer.valueOf(pthTestPlan.getUrgentTest()).toString(), font8));
            table3.addCell(new Paragraph("Regular Tests ", font8));
            table3.addCell(new Paragraph(Integer.valueOf(pthTestPlan.getRegularTest()).toString(), font8));
